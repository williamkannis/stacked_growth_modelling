...
parameters {
  ...
  sum_to_zero_vector[N_CAT]    beta_g2;  // g2 group effect
  sum_to_zero_vector[N_CAT]    beta_ti;  // ti group effect
  
  // Hyperparameters - means
  // Linf and g2 are log-transfomred to improve convergence 
...
transformed parameters {
  ...
  row_vector[N_SITES]           site_g2;  // site-level g2's
  ...
  // Implies multi_normal(mu+beta, Sigma)
  site_Linf = exp(mu_log_Linf+beta_site[1]+to_row_vector(beta_Linf[CAT]));
  site_g2 = exp(mu_log_g2+beta_site[2]+to_row_vector(beta_g2[CAT]));
...
generated quantities{
  ...
  row_vector[N_CAT]      cat_g2;  // group specific g2
  ...
  cat_g2 = exp(mu_log_g2+to_row_vector(beta_g2));
