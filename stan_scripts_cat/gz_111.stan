...
parameters {
  ...
  sum_to_zero_vector[N_HYDR]    beta_gi;  // gi hydroperiod effect
  sum_to_zero_vector[N_HYDR]    beta_ti;  // ti hydroperiod effect
  
  /// Hyperparameters - means ///
  
  // Linf and gi are log-transfomred to improve convergence 
  // and ensure postivevalues
  real                     mu_log_Linf;  // log-transformed Linf hyperparameter
  real                       mu_log_gi;  // log-transformed gi hyperparameter
...
transformed parameters {
  ...
  row_vector[N_SITES]               gi;  // site-level gi's
  ...
  gi = exp(mu_log_gi+beta_site[2]+to_row_vector(beta_gi[HYDR]));
...
model {
  ...
  length_hat = Linf[ID] .* exp(-exp(-gi[ID] .* (AGE - ti[ID])));
...
generated quantities{
  ...
  hydr_gi = exp(mu_log_gi+to_row_vector(beta_gi));
  ...
  for(i in 1:N){
    log_lik[i] = normal_lpdf(LENGTH[i]|Linf[ID[i]] .* exp(-exp(-gi[ID[i]] .* (AGE[i] - ti[ID[i]]))),sigma_length);
  }
}
